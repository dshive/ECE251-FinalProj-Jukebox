...
;***************************************************************
;LABEL		DIRECTIVE	VALUE			COMMENT
			AREA    	MotorDrive, READONLY, CODE
			THUMB
			EXPORT  	__MotorDrive		; Make available
...
;*************************************************************************
			...
			MOV r1, #2_00011110 ; Pull up resistors have a 1 so they are at 1-4
			STR r1, [r0]
...
Motor		
			PUSH		{LR}
			;LDR			R3,=4 this is extra code if we want to step the motor more than one step to fill the circle
			BL			Delay
			BL			Step
			BL			Delay
			BL			Step2
			...
			BL			Step4
			BL			Delay
			;CMP			R3,#0 This is for the extra code
			;BNE			Motor  This is for the extra code
			POP			{PC}
						
			;PF4 of the board connects to IN1 on the motor driver
Step		LDR			R1, =GPIO_PORTF_DATA
			MOV			R0, #0x10
			STR 		R0, [R1]
			BX			LR

Step2		;PF3 of the board connects to IN3 on the motor driver
			LDR			R1, =GPIO_PORTF_DATA
			MOV			R0, #0x08
			STR 		R0, [R1]
			BX			LR
			
Step3		;PF2 of the board connects to IN2 on the motor driver
			LDR			R1, =GPIO_PORTF_DATA
			MOV			R0, #0x04
			STR 		R0, [R1]
			BX			LR
...
Step4	;PF1 of the board connects to IN4 on the motor driver
			LDR			R1, =GPIO_PORTF_DATA
			MOV			R0, #0x02
			STR 		R0, [R1]
			BX			LR
...
;******************************************
; Subroutine to create delay for the motor, 
